...
from transformers import EvalPrediction,AutoModelForSequenceClassification
...
def aggregate_flora(local_param_list, num_sample_list, rank_list, model, args, save_dir):
    start_time = time.time()
    total_samples = sum(num_sample_list)
    global_param = {}
    for n, p in local_param_list[0].items():
        if "lora_B" in n:
            global_param[n] = p.data.clone()
        else:
            global_param[n] = p.data.clone() * (num_sample_list[0]/total_samples)
    for local_param, num_sample, rank in zip(local_param_list[1:], num_sample_list[1:], rank_list[1:]):
        weight = num_sample / total_samples
        for n, p in local_param.items():
            if "lora_A" in n:
                global_param[n] = torch.cat([global_param[n], p.data.clone() * weight], dim=0)
            elif "lora_B" in n:
                global_param[n] = torch.cat([global_param[n], p.data.clone()], dim=-1)
            else:
                global_param[n] += p.data.clone() * weight
    agg_time = time.time() - start_time
    logging.info(f"FLORA Aggregation time: {agg_time:.2f}s")
    ...
    for n, p in model.named_parameters():
        if "lora" not in n:
            torch.save(p.data, os.path.join(save_dir, f"{n}.pth"))
    return model
...
def main_flora(args):
    ...
    model_whole_param_dict_dir = os.path.join(result_dir, "model_whole_param_dict")
    if args.resume and os.path.exists(model_whole_param_dict_dir):
        ...
        start_epoch = 0
        # if model_whole_param_dict_dir exists, remove it
        if os.path.exists(model_whole_param_dict_dir):
            for file in os.listdir(model_whole_param_dict_dir):
                os.remove(os.path.join(model_whole_param_dict_dir, file))
        else:
            os.makedirs(model_whole_param_dict_dir)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for client_id in selected_client_ids:
            ...
            if epoch >= 1:
                for n, p in model.named_parameters():
                    if "lora" not in n:
                        model_whole_param = torch.load(os.path.join(model_whole_param_dict_dir, f"{n}.pth"))
                        p.data.copy_(model_whole_param)
                
            # load new lora
            model = reload_lora_into_model(args.target_modules, model, client_rank, args.model_name, args.dataset, "client")
            ...
            # local training 
            client_train_result = trainer.train()   # trainer moves model into cuda for training (set in TrainingArguments)
        ...
        model = aggregate_flora(local_param_list, num_sample_list, rank_list, model, args, model_whole_param_dict_dir)
    ...
    import json
    with open(f"./results/{args.model_name}/{args.output_name}/final_results.json", "w") as f:
