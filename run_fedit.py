...
def aggregate_homo(local_param_list, num_sample_list, rank_list, global_param, model):
    ...
    logging.info(f"HOMO Aggregation time: {agg_time:.2f}s")
...
def load_global_param_homo(args, model, global_param, model_name, client_rank, dataset_name):
    model = reload_lora_into_model(args.target_modules, model, client_rank, model_name, dataset_name, "client")
...
def main_homo(args):
    ...
    min_rank = min(involved_ranks)

    model, tokenizer = load_model_and_tokenizer(args.model_name, args.dataset)
    model = reload_lora_into_model(args.target_modules, model, min_rank, args.model_name, args.dataset, "client")
    model.set_adapter("client")
    ...
    # set up client ranks
    client_ranks_list = [min_rank for _ in range(args.num_clients)]
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for client_id in selected_client_ids:
            ...
            client_rank = client_ranks_list[client_id]
            ...
            model = load_global_param_homo(args, model, global_param, args.model_name, client_rank, args.dataset)
        ...
        model, global_param = aggregate_homo(local_param_list, num_sample_list, rank_list, global_param, model)
