...
def aggregate_zeropad(local_param_list, num_sample_list, rank_list, global_param, model):
    global_param = {n:torch.zeros_like(p) for n, p in global_param.items()}
    for n, global_p in global_param.items():
        for local_param, num_sample, rank in zip(local_param_list, num_sample_list, rank_list):
            if "lora_A" in n:
                global_p[:rank, :] += local_param[n] * num_sample / sum(num_sample_list)
            elif "lora_B" in n:
                global_p[:, :rank] += local_param[n] * num_sample / sum(num_sample_list)
            else:
                global_p += local_param[n] * num_sample / sum(num_sample_list)
...
def load_global_param_zeropad(target_modules, model, global_param, client_rank, model_name, dataset_name):
...
def main_zeropad(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for client_id in selected_client_ids:
            ...
            model = load_global_param_zeropad(args.target_modules, model, global_param, client_rank, args.model_name, args.dataset)
        ...
        model, global_param = aggregate_zeropad(local_param_list, num_sample_list, rank_list, global_param, model)

        # evaluate aggregated model
        trainer = get_trainer(args, model, epoch_lr, train_dataset, validation_dataset, tokenizer, compute_metrics)
